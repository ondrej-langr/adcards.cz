...
class NewsletterController
{
    ...
    #[AsApiRoute('POST', '/newsletter/subscribe', 'newsletterSubscribe')]
    public function subscribe(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
    {
        $body = $request->getParsedBody();
        $config = $this->container->get(Config::class);
        $success = false;
        $service = new EntryTypeService(new \PromCMS\App\Models\PromoCodes());
        $payload = [
            'code' => uniqid(8),
            'amount' => 10,
            'enabled' => true,
        ...
        $this->container->get(RenderingService::class)->render($response, '@app/layouts/site-layout/footer-newsletter-subscribe.twig', [
            'success' => $success,
        ]);
